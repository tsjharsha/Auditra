...
export function WorldsPage() {
  ...
  const [adapter, setAdapter] = useState<"json" | "gateway_test">("json");
  ...
  return <div className="space-y-6 pb-8">
    <header className="rise-in border-b border-white/10 pb-5">
      <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
        <div><StatusPill accent="cyan" dot>Batch workspace</StatusPill><h1 className="mt-4 text-2xl font-semibold text-white">Prepare the batch before the close.</h1><p className="mt-3 max-w-3xl text-sm leading-6 text-[#9a9792] sm:text-base">Generate a measurable finance-operations scenario, or validate the shape of an external test batch before connecting it to a production adapter.</p></div>
    ...
    <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_400px]">
      <WorkspacePanel>
        ...
        <div className="mt-4 grid gap-2 lg:grid-cols-3">{PROMPT_SUGGESTIONS.map((item) => <button key={item} type="button" className="border-b border-white/[0.08] p-3 text-left text-xs leading-5 text-[#9a9792] hover:border-[#c7ff54]/40 hover:text-white" onClick={() => setPrompt(item)}>{item}</button>)}</div>
    ...
    <WorkspacePanel>
      <SectionTitle icon={<Upload className="h-5 w-5" />} eyebrow="Import test data" title="Validate an external batch shape" detail="JSON and Gateway-style test payloads are normalized through the same canonical schema. Imported data is validated but is not presented as a hidden-truth benchmark." />
      <div className="mt-5 grid gap-5 xl:grid-cols-[minmax(0,1fr)_320px]">
        <div><div className="mb-3 flex flex-wrap gap-2"><button type="button" className={"record-option px-4 " + (adapter === "json" ? "record-option-active" : "")} onClick={() => setAdapter("json")}>Canonical JSON</button><button type="button" className={"record-option px-4 " + (adapter === "gateway_test" ? "record-option-active" : "")} onClick={() => setAdapter("gateway_test")}>Gateway-style JSON</button></div><textarea className="min-h-[220px] w-full resize-y rounded-lg border border-white/10 bg-black/20 p-4 font-mono text-xs leading-5 text-[#c7c4bf]" value={importText} onChange={(event) => setImportText(event.target.value)} /><button type="button" className="button-secondary mt-4" disabled={importing} onClick={() => void validateImport()}>{importing ? <RefreshCw className="h-4 w-4 animate-spin" /> : <CheckCircle2 className="h-4 w-4" />}{importing ? "Validating" : "Validate batch"}</button>{importError ? <div className="mt-3 text-sm text-[#ffb08d]">{importError}</div> : null}</div>
        <div className="border-l border-white/10 pl-4">{ingestion ? <><div className="section-kicker">Normalized</div><div className="mt-2 text-lg font-semibold text-white">Batch accepted</div><div className="mt-5 space-y-3">{Object.entries(ingestion.rows_loaded).map(([label, value]) => <div key={label} className="flex justify-between text-sm"><span className="text-[#9a9792]">{titleCase(label)}</span><span className="font-semibold text-white">{compact(value)}</span></div>)}</div><p className="mt-5 text-xs leading-5 text-[#77736e]">Use a controlled scenario for hidden-truth accuracy and assurance. External data retains the same adapter boundary for a later production integration.</p></> : <><div className="section-kicker">Adapter boundary</div><p className="mt-3 text-sm leading-6 text-[#9a9792]">The importer accepts payments, orders, settlements, refunds, fees, and merchants, then validates references and finance data types before controller integration.</p></>}</div>
      </div>
    </WorkspacePanel>
